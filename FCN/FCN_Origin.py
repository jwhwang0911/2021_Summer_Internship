...
from GetData import dataloader,bag
import pdb
import numpy as np 
import time
import visdom
...
if __name__ == "__main__":
    ...
    fcn_model = FCN32s(pretrained_net=vgg_model,n_class=3)
    ...
    with torch.no_grad():
        ...
        dataloader = DataLoader(bag, batch_size=1,shuffle=False,drop_last=True)
        ...
        for item in dataloader:
            ...
            input_np = input_np.swapaxes(1,2).swapaxes(0,2)
        ...
        elem_input = input_list[0]
        elem_output = output_list[0]
        
        for i in range(1,len(input_list)):
            if i % 9 == 0:
                exr_input.append(elem_input)
                elem_input = input_list[i]
            else:
                elem_input = np.concatenate((elem_input,input_list[i]),axis=0)
        ...
        final_input = exr_input[0]
        final_output = exr_output[0]
        
        for e_idx in range(1,len(exr_input)):
            final_input = np.concatenate((final_input,exr_input[e_idx]),axis = 1)
        ...
        # item = test_set.__getitem__(0)
        # input = item['A'].cuda()
        # print("input:",input.shape)
        # input = fcn_model(input)
        # print("input:",input.shape)
        # y = input.cpu().data.numpy().copy()
        # elem = y.swapaxes(1,2).swapaxes(0,2)
        
        # for idx in range(1,test_set.__len__()):
        #     item = test_set.__getitem__(idx)
        #     input = item['A']
        #     input = fcn_model(input)
        #     y = input.cpu().data.numpy().copy()
        #     y = y.swapaxes(1,2).swapaxes(0,2)
        #     if idx % 9 == 0:
        #         input_list.append(elem)
        #         elem = y
        #     else:
        #         elem = np.concatenate((elem,y),axis=0)
                
        # final_output = input_list[0]
        # for e_idx in range(1,len(input_list)):
        #     final_output = np.concatenate((final_output,input_list[e_idx]),axis = 1)
        
        exr.write("/rec/pvr1/deep_learning_denoising/test_write/"+"test_input.exr",
                   final_input
                   )
