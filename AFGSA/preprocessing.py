...
# absolute path of gt & noisy input
def preprocessing(nsy_path:torch.Tensor, gt_path:torch.Tensor)->dict[str,torch.Tensor]:
    # data_keys = ["noisy", "aux", "gt", "normal", "depth", "albedo"]
    data = {"noisy" : None, "aux" : None, "gt":None, 'normal':None, 'depth':None, 'albedo':None}
    d = exr.read_all(nsy_path)
    d_gt = exr.read_all(nsy_path)
    data["noisy"] = clip_numpy(d["default"])
    data["gt"] = clip_numpy(d_gt["default"])
...
d = preprocessing("/rec/pvr1/2021_Summer_Internship/raw_data/train_set/noisy/10064185-00128spp.exr","/rec/pvr1/2021_Summer_Internship/raw_data/train_set/ground_truth/10064185-08192spp.exr")
importance_sampling(d, 64,400 )